    ...
    def play_round(self):
        ...
        while self._user.make_move():
            ...
            if self._user.get_hand().get_score() > 21:
                print(
                    f"User score is over 21. You lose! Your current balance is {self._user.get_money()}"
                )
                self.reset_game()
                return
            else:
        ...
        while self._dealer.make_move():
            ...
            if self._dealer.get_hand().get_score() > 21:
                ...
                print(
                    f"Dealer score is over 21. You win! Your current balance is {self._user.get_money()}"
                )
                self.reset_game()
                return
            else:
